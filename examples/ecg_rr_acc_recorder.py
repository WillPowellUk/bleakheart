...
async def run_ble_client(device, ecg_acc_queue, hr_queue, quitclient):
    ...
    try:
        async with BleakClient(device, disconnected_callback=disconnected_callback) as client:
            ...
            pmd = PolarMeasurementData(client, ecg_queue=ecg_acc_queue, acc_queue=ecg_acc_queue)
            ...
            for k, v in settings.items():
                logger.info(f"{k}:\t{v}")
            try:
                err_code, err_msg, *_ = await pmd.start_streaming('ACC', RANGE=2, SAMPLE_RATE=25)
                if err_code != 0:
                    logger.error(f"PMD returned an error: {err_msg}")
                    sys.exit(err_code)
            except Exception as e:
            ...
            ecg_acc_queue.put_nowait(('QUIT', None, None, None))
...
async def run_consumer_task_ecg_acc(ecg_acc_queue, file_path):
    """Retrieve ECG/ACC data from the queue and store it in a CSV file."""
    ...
    with open(file_path_ecg, 'w', newline='') as ecg_file,\
        open(file_path_acc, 'w', newline='') as acc_file:
    
        ecg_writer = csv.writer(ecg_file)
        acc_writer = csv.writer(acc_file)

        while True:
            frame = await ecg_acc_queue.get()
            type = frame[0]
            match type:
                case 'ECG':
                ...
                case 'ACC':
                    timestamp_data, acc_data = frame[1], frame[2]
                    for sample in acc_data:
                        acc_writer.writerow([*sample])
                case 'QUIT':
                    break
                case _:
...
async def main(file_path):
    ...
    if device is None:
        ...
        sys.exit(-4)
    ecg_acc_queue = asyncio.Queue()
    hr_queue = asyncio.Queue()
    quitclient = asyncio.Event()
    input_thread_obj = threading.Thread(target=input_thread, args=(quitclient,))
    input_thread_obj.start()
    producer = run_ble_client(device, ecg_acc_queue, hr_queue, quitclient)
    consumer_ecg_acc = run_consumer_task_ecg_acc(ecg_acc_queue, file_path)
    consumer_hr = run_consumer_task_hr(hr_queue, file_path)
    await asyncio.gather(producer, consumer_ecg_acc, consumer_hr)
    logger.info("Bye.")
